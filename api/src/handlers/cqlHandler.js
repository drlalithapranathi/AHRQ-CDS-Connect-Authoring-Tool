...
module.exports = {
  ...
  formatCQL,
  buildCQL,
  // NEW: Export sync functions for syncing to CQL Services
  syncToCqlServices
};
...
function objConvert(req, res, callback) {
  ...
  // NEW: Check if we should sync to CQL Services
  ...
  CQLLibrary.find({ ...libraryFilter, linkedArtifactId: { $ne: null, $eq: artifactId } })
    ...
    .then(libraries => {
      ...
      // NEW: Attach sync flag to artifact object for use in writeZip
      artifact._syncToCqlServices = syncToCqlServicesFlag;
...
// Save to CQL Services
      ...
      // NEW: Sync to CQL Services if requested
      if (artifact._syncToCqlServices) {
        console.log('Syncing to CQL Services...');
        const syncResult = syncToCqlServices(artifact, elmFiles);
        if (syncResult.success) {
          console.log('✓ Sync to CQL Services completed');
        } else {
          console.error('✗ Sync to CQL Services failed:', syncResult.message);
        }
      }
...
// NEW: Function to sync artifact to CQL Services
// Writes ELM JSON and Hook config to CQL Services folders
function syncToCqlServices(artifact, elmFiles) {
  if (!config.get('cqlServices.syncEnabled')) {
    console.log('CQL Services sync is disabled in config');
    return { success: false, message: 'Sync disabled in config' };
  }

  try {
    const artifactName = slug(artifact.name ? artifact.name : 'untitled', { lower: false });
    const librariesPath = config.get('cqlServices.librariesPath');
    const hooksPath = config.get('cqlServices.hooksPath');

    // Ensure CQL Services paths exist
    if (!fs.existsSync(librariesPath)) {
      return { success: false, message: 'CQL Services libraries path not found: ' + librariesPath };
    }
    if (!fs.existsSync(hooksPath)) {
      return { success: false, message: 'CQL Services hooks path not found: ' + hooksPath };
    }

    // Create artifact directory in libraries folder
    const artifactLibDir = path.join(librariesPath, artifactName);
    if (!fs.existsSync(artifactLibDir)) {
      fs.mkdirSync(artifactLibDir, { recursive: true });
    }

    // NEW: Write ELM JSON files to libraries folder
    // Filter to only process JSON files (exclude XML files that start with '<')
    const elmJsonFiles = elmFiles.filter(elmFile => !elmFile.content.startsWith('<'));
    elmJsonFiles.forEach(elmFile => {
      const elmJson = JSON.parse(elmFile.content);
      const libraryId = elmJson.library.identifier.id;
      const elmFilePath = path.join(artifactLibDir, `${libraryId}.json`);
      fs.writeFileSync(elmFilePath, JSON.stringify(elmJson, null, 2), 'utf8');
      console.log(`✓ Synced ELM to CQL Services: ${artifactName}/${libraryId}.json`);
    });

    // NEW: Generate and write Hook configuration
    const hookConfig = generateHookConfig(artifact, elmJsonFiles);
    const hookFilePath = path.join(hooksPath, `${artifactName}.json`);
    fs.writeFileSync(hookFilePath, JSON.stringify(hookConfig, null, 2), 'utf8');
    console.log(`✓ Synced Hook config to CQL Services: ${artifactName}.json`);

    return {
      success: true,
      message: 'Successfully synced to CQL Services',
      files: {
        libraries: elmJsonFiles.map(e => `${artifactName}/${JSON.parse(e.content).library.identifier.id}.json`),
        hook: `${artifactName}.json`
      }
    };
  } catch (err) {
    console.error('✗ Error syncing to CQL Services:', err);
    return { success: false, message: err.message };
  }
}

// NEW: Generate CDS Hook configuration from artifact
function generateHookConfig(artifact, elmFiles) {
  const artifactName = slug(artifact.name ? artifact.name : 'untitled', { lower: false });

  // Get the main ELM file (first one, which is the artifact itself)
  const mainElm = JSON.parse(elmFiles[0].content);
  const libraryId = mainElm.library.identifier.id;
  const libraryVersion = mainElm.library.identifier.version;

  // NEW: Build cards from recommendations
  const cards = [];
  if (artifact.recommendations && artifact.recommendations.length > 0) {
    artifact.recommendations.forEach((rec, index) => {
      const card = {
        conditionExpression: 'InPopulation', // Default, could be customized per recommendation
        card: {
          summary: artifact.name || 'Clinical Decision Support',
          indicator: 'info', // Could be 'info', 'warning', 'critical'
          detail: '${Recommendation}', // CQL expression reference
          source: {
            label: `CDS Connect: ${artifact.name}`,
            url: 'https://cds.ahrq.gov/cdsconnect'
          }
        }
      };

      // NEW: Add rationale if available
      if (rec.rationale) {
        card.card.extension = {
          rationale: '${Rationale}'
        };
    ...
    // NEW: Default card if no recommendations
    cards.push({
  ...
  // NEW: Generate hook configuration
  const hookConfig = {
    id: artifactName,
    hook: 'patient-view', // Default hook type
    title: artifact.name || 'Clinical Decision Support',
    description: artifact.description || `Clinical decision support for ${artifact.name}`,
    _config: {
      cards: cards,
      cql: {
        library: {
